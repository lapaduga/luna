...
> Обновление: 2026-08-11. Пункты 1–2 закрыты правками по отчёту партнёра
> (см. `reports/DAY13_PARTNER_FIXES.md`): soft hyphen, homoglyph, fullwidth
> и base64x2 теперь ловятся, перефразы инъекций — тоже.

1. ~~**Homoglyph-ключ** — `sк-…` (кириллическая `к`) НЕ ловился regex-детектами~~.
   **Закрыто**: `normalizeForDetection()` сводит кириллические homoglyphs и
   fullwidth к ASCII, `sanitizeText` вырезает soft hyphen — обфусцированный ключ
   блокируется (403). Остаются «новые» confusables вне таблицы — расширяемая.
2. ~~**base64url** (`-`/`_` вместо `+/`)~~. **Частично закрыто**: `scanBase64`
   теперь рекурсивный (глубина 3), ловит base64x2; маскировка base64-блобов
   реально работает (`REDACTED_BASE64`).
3. **`GUARD_LLM=0`** — medium-риски инъекций пропускаются эвристиками, а
   перефразы теперь помечаются `review` и при `GUARD_LLM=1` уходят в
   LLM-классификатор. Рекомендация: включить `GUARD_LLM=1`.
4. **L1 zero-width** — `sanitizeText` вырезает zero-width и soft hyphen, но
   изощрённый текст (комбинация с форматированием) может просочиться сквозь
   простую эвристику (см. День 12: спасают L2/L3, т.е. границы + output guard).
